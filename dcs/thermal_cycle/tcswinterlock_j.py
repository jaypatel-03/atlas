...
MODULES = [2,3,4] # which modules to use
DB_TOKEN = 'REDACTED' # Token for the InfluxDB database, redacted for security
N_CYCLES = 10
MAX_TEMP = 40 # Maximum temperature for the Peltier elements in °C
MIN_TEMP = -45 # Minimum temperature for the Peltier elements in °C
PELTIER_CONFIGS = [f"../configs/pidcontroller_j{m}.toml" for m in MODULES] # Configuration files for the Peltier controllers
### END OF MODIFICATION ###
...
def interlock_test(base, pelts, ntcs, chuck_temp, lid, humi, temp_85, mini_ramp_up):
    ...
    if any([t > 70 for t in ntcs.value]):
        ...
        return True, 'Temperature', mini_ramp_up
    ...
    if any([dewpoint > ch_t - 2 for ch_t in chuck_temp.value]):
        ...
        return True, 'Dewpoint', mini_ramp_up
    ...
    if lid.value < 4:
        ...
        pelts_on_off(pelts, False)
        return True, 'Open Lid', mini_ramp_up
    return False, '', mini_ramp_up
...
def ramp_down(fl, interlock_condition, ntcs, lvs, pelt_psu, hvs, humi, temp, chuck_temp : list, chiller, pelts : list, base, HEADER, write_api, lid, temp_85, mini_ramp_up, min_temp):
    ...
    cause = ''
    logging.info("TRYING TO RAMP DOWN, TURN pelt_psu ON")
...
def main():
    ...
    pelts = connect_pelts(port0=19896)
    if not pelts:
...
def main_with_instruments(ntcs, lvs, pelt_psu, hvs, humi, chuck_temp, chiller, pelts, base, lid, temp_85, **kwargs):
    ...
    with open(file_path,'a') as fl: #FOLLOW EXAMPLE IN MAIN, NEED TO WRAP AROUND POLL PROCESS AND WRITE LINE BY LINE
        ...
        cycles = 0
        max_cycles = kwargs.get('n_cycles', N_CYCLES)
        max_temp = kwargs.get('max_temp', MAX_TEMP)
        min_temp = kwargs.get('min_temp', MIN_TEMP)
        MODULES = kwargs.get('modules', MODULES)
