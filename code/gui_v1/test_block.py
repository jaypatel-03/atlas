...
class TestBlock(tk.Frame):
    ...
    def run_test(self, master, button : tk.Button, test : str, mod_data : ModuleTestData):
        ...
        if loc_id is not None:
            ...
            if test in ['IV-MEASURE', 'ADC-CALIBRATION', 'ANALOG-READBACK', 'SLDO', 'VCAL-CALIBRATION', 'INJECTION-CAPACITANCE', 'LP-MODE', 'DATA-TRANSMISSION']:
                template = "echo cd {home_path}/module-qc-tools ; echo measurement-{test} -c ../configs/new_hw_config_{version}.json -m ../module-qc-database-tools/{loc_id}/{mod_sn}/{mod_sn}_L2_{temp}.json ; sleep 3"
            elif test == "eyeDiagram":
                template = "echo cd {home_path}/Yarr ; echo bin/eyeDiagram -r configs/controller/specCfg-rd53b-16x1.json -c ../module-qc-database-tools/{loc_id}/{mod_sn}/{mod_sn}_L2_{temp}.json > /home/jayp/atlas/code/gui_v1/logs/eyeDiagram.log ; sleep 3" # TODO: change output directory
            else:
                template = "echo cd {home_path}/Yarr ; echo bin/scanConsole -r configs/controller/specCfg-rd53b-16x1.json -c ../module-qc-database-tools/{loc_id}/{mod_sn}/{mod_sn}_L2_{temp}.json -s configs/scans/rd53b/{test} -Wh ; sleep 3"
    ...
    def make_buttons(self, master, tests : list, mod_data : ModuleTestData):
        ...
        for test in tests:
            ...
            quick_btn = tk.Button(self, text=f"Run {test}", command=lambda r=r: self.run_test(master,test_buttons[r],tests[r], mod_data))
    ...
    def open_popup(self, master, test : str, cmd : str):
        ...
        tk.Label(popup, text=f"Running {test}, will take approx {self._TEST_TIMES[test]} mins. \n ETA: {(datetime.datetime.now()+datetime.timedelta(minutes=int(self._TEST_TIMES[test]))).strftime('%H:%M')}").pack(fill='x', pady=10)
